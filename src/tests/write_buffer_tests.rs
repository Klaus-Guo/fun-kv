...
use crate::{core::record::Record, error::DbError, stats::Statistics, storage::{free_space_manager::FreeSpaceManager, write_buffer::{WriteBuffer, WriteEntry}}};
...
fn test_write_buffer_shutdown() {
    ...
    assert!(matches!(result.unwrap_err(), DbError::ShuttingDown));
}
...
fn test_write_entry_fields() {
    ...
    let entry = WriteEntry {
        operation: crate::constants::Operation::Insert,
        record: record.clone(),
        old_value_len: 0,
