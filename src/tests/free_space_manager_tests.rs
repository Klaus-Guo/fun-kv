use crate::{constants::*, error::DbError, storage::free_space_manager::FreeSpaceManager};
...
fn test_initialization() {
    ...
    let device_size = DEFAULT_PERSISTENT_SIZE as u64;
    ...
    let expected_free = device_size - (16 * BLOCK_SIZE as u64) ;
...
fn test_allocate_and_release() {
    let mut manager = FreeSpaceManager::new();
    manager.initialize(DEFAULT_PERSISTENT_SIZE as u64).unwrap();
...
fn test_best_fit_allocation() {
    ...
    manager.set_persistence_size(10000 * BLOCK_SIZE as u64);
...
fn test_coalescing() {
    ...
    assert_eq!(manager.get_free_chunks_count(), 1);
    assert_eq!(manager.get_total_free(), 150 * BLOCK_SIZE as u64);
}
...
fn test_invalid_operations() {
    ...
    let huge = (DEFAULT_PERSISTENT_SIZE / BLOCK_SIZE) as u64 + 1;
...
fn test_fragmentation_calculation() {
    ...
    assert_eq!(manager.get_fragmentation_percent(), 0);

    // Add another chunk - now fragmented
    manager.release_sectors(2000, 100).unwrap();
    assert!(manager.get_fragmentation_percent() > 0);
    ...
    assert!(manager.get_fragmentation_percent() > 10); // Some fragmentation
}
...
fn test_out_of_space() {
    let mut manager = FreeSpaceManager::new();
    manager.initialize(1024 * BLOCK_SIZE as u64).unwrap();
    ...
    assert!(matches!(result.unwrap_err(), DbError::OutOfSpace));
}

#[test]
fn test_device_bounds_checking() {
    let mut manager = FreeSpaceManager::new();
    manager.set_persistence_size(1000 * BLOCK_SIZE as u64);
...
fn test_concurrent_allocation() {
    ...
    let manager = Arc::new(Mutex::new({
        ...
        m.initialize(DEFAULT_PERSISTENT_SIZE as u64).unwrap();
        m
    }));
...
fn test_coalesce_with_successor() {
    ...
    assert_eq!(manager.get_free_chunks_count(), 1);
    assert_eq!(manager.get_total_free(), 100 * BLOCK_SIZE as u64);
}
