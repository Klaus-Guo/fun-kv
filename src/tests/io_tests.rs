...
use crate::{constants::*, storage::io::DiskIO};
...
fn create_test_disk_io() -> (DiskIO, NamedTempFile) {
    ...
    // Set file size
    file.set_len(DEFAULT_PERSISTENT_SIZE as u64).unwrap();
...
fn test_multiple_sector_read_write() {
    ...
    let sectors = 5;
    let test_data = vec![123u8; BLOCK_SIZE * sectors];
...
fn test_sector_alignment() {
    ...
    let test_data = vec![0xABu8; BLOCK_SIZE];
    let sector = 123;
...
fn test_edge_sectors() {
    ...
    let first_data_sector = DATA_START_BLOCK;
    let test_data = vec![0x11u8; BLOCK_SIZE];
